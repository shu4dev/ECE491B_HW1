...
def main(args):
    ...
    # Initialize wandb with the project name and config parameters.
    wandb.init(project=args.wandb_project, config=vars(args))
    device = torch.device("cuda" if torch.cuda.is_available() and not args.no_cuda else "cpu")
    print(f"Using device: {device}")
    
    # Use np.memmap to load large training and validation datasets.
    train_dataset = np.memmap(args.train_file)
    val_dataset = np.memmap(args.val_file)

    # Initialize the model.
    model = transformer_lm_ablation(
    ...
    # Define loss, optimizer, and other training components.
    ...
    for iteration in range(1, total_iters + 1):
        ...
        # Update learning rate using a cosine schedule.
        ...
        if iteration % args.log_interval == 0:
            ...
            with torch.no_grad():
                ...
                # Calculate perplexity as the exponential of the validation loss.
                val_perplexity = torch.exp(val_loss).item()
                print(f"Iteration {iteration}: Validation Loss = {val_loss.item():.4f}, Perplexity = {val_perplexity:.4f}")
                wandb.log({
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_file', type=str, required=True, 
                        help="Path to training dataset (.npy)")
    parser.add_argument('--val_file', type=str, required=True, 
                        help="Path to validation dataset (.npy)")
    parser.add_argument('--checkpoint_dir', type=str, default="./checkpoints", 
                        help="Directory to save checkpoints")
    
    parser.add_argument('--wandb_project', type=str, default="transformer-parallel", 
